...
public class sampleController {
    ...
    public ResponseEntity<Object> savePatient(@RequestBody Patient patient) {
        if(services.isPatientExist(patient.getId())) {
        	return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Patient ID : " + patient.getId() + " is Already exist");
        }else {
        	Patient createdPatient = services.savePatient(patient);
